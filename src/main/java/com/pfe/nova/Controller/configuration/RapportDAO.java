...
public boolean update(Rapport rapport){
    String sql="UPDATE rapport_detat SET patient_id=?, medecin_id=?, age=?, date_rapport=?, sexe=?, tension_arterielle=?, pouls=?, temperature=?, saturation_oxygene=?, imc=?, niveau_douleur=?, traitement=?, dose_medicament=?, frequence_traitement=?, perte_de_sang=?, temps_operation=?, duree_seance=?, filtration_sang=?, creatinine=?, score_glasgow=?, respiration_assistee=?, complications=? WHERE id=?";
    try(Connection connection= DatabaseConnection.getConnection();
        PreparedStatement stmt=connection.prepareStatement(sql)){
