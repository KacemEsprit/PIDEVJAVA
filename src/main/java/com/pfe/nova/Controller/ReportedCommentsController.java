...
public class ReportedCommentsController {
    ...
    private void handleIgnoreReport(int reportId, int commentId) {
        try {
            ...
            boolean deleted = CommentReportDAO.delete(reportId);

            if (deleted) {
                // Refresh the reports list
                loadReportedComments();
                showAlert(Alert.AlertType.INFORMATION, "Success", "Report has been ignored successfully.");
            } else {
                showAlert(Alert.AlertType.ERROR, "Error", "Failed to ignore report.");
            }
        } catch (SQLException e) {
            System.err.println("Error ignoring report: " + e.getMessage());
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Database Error", "Error ignoring report: " + e.getMessage());
        }
    }
    ...
    private void loadReportedComments() {
        try {
            ...
            reportedCommentsContainer.getChildren().clear();

            // Get all reports from the database
            List<Map<String, Object>> reports = CommentReportDAO.findAllReports();

            if (reports.isEmpty()) {
                Label noReportsLabel = new Label("No reported comments found.");
                noReportsLabel.getStyleClass().add("no-data-label");
                reportedCommentsContainer.getChildren().add(noReportsLabel);
            } else {
                // Create a card for each report
                for (Map<String, Object> report : reports) {
                    VBox reportCard = createReportCard(report);
                    reportedCommentsContainer.getChildren().add(reportCard);
                }
            }
        } catch (SQLException e) {
            System.err.println("Error loading reported comments: " + e.getMessage());
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Database Error", "Error loading reported comments: " + e.getMessage());
        }
    }
    ...
    @FXML
    private VBox reportedCommentsContainer;
}